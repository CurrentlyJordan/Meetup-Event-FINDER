...
public interface MeetupService {
   // https://api.meetup.com/2/open_events?&sign=true&photo-host=public&zip=11222&fields=group_photo&page=20&offset=0&key=%7BYOUR_KEY
    ...
    @GET("/2/open_events")
    Call<MeetupResponse> getMeetupList(@Query ("sign") String sign, @Query("photo-host") String photoHost,
                                       @Query("zip") String zip, @Query("fields") String fields, @Query("page") String page,
                                       @Query("offset") String offset, @Query("key") String key);
}
